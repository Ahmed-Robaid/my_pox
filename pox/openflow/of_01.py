...
def handle_PACKET_OUT (con, msg): #A
  e = con.ofnexus.raiseEventNoErrors(PacketOut, con, msg)
  if e is None or e.halt != True:
    con.raiseEventNoErrors(PacketOut, con, msg)
  ...
  def send (self, con, data):
    with self._lock:
      ...
      if isinstance(data, of.ofp_packet_out):
      	log.debug("Connection.send - OFP_PACKET_OUT")
      	handle_PACKET_OUT(self, data)
  ...
  def send (self, data):
    ...
    if type(data) is not bytes:
      ...
      data = data.pack()
    	...
    	log.debug("Connection.send - type(data is bytes!")
